...
var DOMsay = select('.say');
var DOMsmiley = select('.smiley');
var DOMplugin = select('.plugin_name');
...
function Recognizer() {
  ...
  this.confidence = select( '.speech .confidence' )
  this.transcript = select( '.speech .transcript' )
...
window.addEventListener( 'DOMContentLoaded', function() {
	...
	DOMsay.textContent = '...';
	idleAnim();
	//setInterval(function() {sendData(select( '.speech .transcript' ).textContent,select( '.speech .confidence' ).textContent,"reco", true);},1000);
})
...
function scribeEvents(ev) {
	...
	switch (obj.command) {
		case 'refresh':
			...
			break;
		case 'forceSend':
			//sendData(select( '.speech .transcript' ).textContent,select( '.speech .confidence' ).textContent,"reco", true);
			break;
		case 'abort':
